...
drm_info_bufs(struct tcb *const tcp, const kernel_ulong_t arg)
{
	...
	if (entering(tcp)) {
		tprints("{list=");
		print_array(tcp, (kernel_ulong_t) info.list, info.count,
			    &desc, sizeof(desc),
...
drm_map_bufs(struct tcb *const tcp, const kernel_ulong_t arg)
{
	...
	if (!syserror(tcp) && !umove(tcp, arg, &map)) {
		tprints(", list=");
		print_array(tcp, (kernel_ulong_t) map.list, map.count,
			    &pub, sizeof(pub),
...
drm_free_bufs(struct tcb *const tcp, const kernel_ulong_t arg)
{
	...
	tprints(", list=");
	print_array(tcp, (kernel_ulong_t) free.list, free.count,
	      ...
	      const bool is_get)
{
	...
	if (!syserror(tcp) && !umove(tcp, arg, &map) && is_get) {
		PRINT_FIELD_PTR(", ", map, handle);
	}
...
drm_res_ctx(struct tcb *const tcp, const kernel_ulong_t arg)
{
	...
	tprints(", contexts=");
	print_array(tcp, (kernel_ulong_t) ctx.contexts, ctx.count,
		    &ctx, sizeof(ctx),
...
drm_wait_vblank(struct tcb *const tcp, const kernel_ulong_t arg)
{
	...
	if (entering(tcp)) {
		...
		PRINT_FIELD_XVAL("{request={", vblank.request, type,
				 drm_vblank_seq_type, "_DRM_VBLANK_???");
	...
	if (!syserror(tcp) && !umove(tcp, arg, &vblank)) {
		PRINT_FIELD_XVAL(", reply={", vblank.reply, type
				 drm_vblank_seq_type, "_DRM_VBLANK_???");
